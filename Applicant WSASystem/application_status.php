...
$select = mysqli_query($dbConn, "SELECT application_id, scholarship_name, date_submitted, status, 'tbl_userapp' AS source FROM tbl_userapp WHERE user_id = '$user_id'
                                UNION
                                SELECT application_id, scholarship_name, date_submitted, status, 'tbl_scholarship_1_form' AS source FROM tbl_scholarship_1_form WHERE user_id = '$user_id'") or die(mysqli_error($dbConn));
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        while ($row = mysqli_fetch_array($select)) {
                            ...
                            if ($row['source'] === 'tbl_userapp') {
                                $updateLink = 'update_details.php';
                            } elseif ($row['source'] === 'tbl_scholarship_1_form') {
                                $updateLink = 'update_scholarship_1_details.php';
                            } else {
                                // Handle other cases or sources as needed
                                $updateLink = '#'; // Placeholder or fallback link
                            }
                            ...
                            <tr>
                                ...
                                <td>
                                    <strong><a class="view-link" href="' . $updateLink . '?source=' . $row['source'] . '&id=' . $row['application_id'] . '">View</a></strong>
                                </td>
                            </tr>';
                        }
                        ?>
